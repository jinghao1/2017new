...
//        console.log("分 组1:"+brand_year_car_choice_data+",分 组2:"+brand_type_car_choice_data+"分 组3:"+brand_num_choice_data);
    ...
    //点击对比按钮，为了兼容旧dom
    $('.brand_contrast_table').on('click','a.contrast_btn',function(e){
        e.stopPropagation();
        var id = $(this).attr('id')
    ...
    $('.car_type_summar').on('click','a.contrast_btn',function(e){
        e.stopPropagation()
        var id = $(this).attr('id')
        var text = $(this).attr('text')
...
var carSelecter = function(opt){
    ...
    function bindEvents(){  
        $('.type_constract_list').on('click',function(){  
...
var carCompareLayer = (function(){ 
    ...
    function renderList(){ 
        ...
        for(var i=0;i<cache.length;i++){    
            html += '<li><a target="_blank" href="http://car.qichedaquan.com/carparam/'+ cache[i].id +'">'+ cache[i].text +'</a><i class="delete_icon"></i></li>'
            link += ',' + cache[i].id
        }
    ...
    function bindEvents(){ 
        $('.type_constract')
            ...
            .on('click','.js_hide',function(event){$('.js_con_list').hide()})
            .on('click','.js_constract',function(event){$('.js_con_list').toggle()})
            .on('click',function(e){
                e.stopPropagation()
            })
        //点击页面车型对比弹出框消失
...
var citySelect = function (target,city,current) {
	var citySelect=(function (target,city) {
		...
		$('#city_chose .city_chose_region dd a').on('click',function () {
		...
		$('#city_chose .city_hot_search a').on('click',function () {
			var serial_val = $("#serialId").attr("value");
			var caryear_val = $("#caryear").attr("value");
			var serialId = (serial_val==undefined||serial_val==null||serial_val=="")?0:serial_val;
			var caryear = (caryear_val==undefined||caryear_val==null||caryear_val=="")?0:caryear_val;
...
//城市模块hover显示，离开隐藏
	var cityChange=(function (current,city) {
		$('#'+city+' .city_chose_region dd a').on('click',function () {
			...
			$(".default_address a").html($(this).text());
			flag=false;
		})
	...
	$('#'+city+' .city_chose_region').css("marginTop","0px");
